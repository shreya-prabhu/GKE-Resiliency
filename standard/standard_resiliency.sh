...
FAILOVER_ZONES=$(gcloud compute zones list --filter="name ~ '$REGION.*' AND name != '$FAILURE_ZONE'" --format="value(name)" | tr '\n' ',') FAILOVER_ZONES=${FAILOVER_ZONES%,} # Remove the trailing comma
echo -e "\nExcluding the failure zone $FAILURE_ZONE, the available zones are: $FAILOVER_ZONES"

sleep 5

echo -e "\nUpdating node pool to restrict nodes to zones $FAILOVER_ZONES, excluding the target failure zone ($FAILURE_ZONE)..."
echo " gcloud container node-pools update $NODE_POOL --cluster=$GKE_CLUSTER_NAME --node-locations=$FAILOVER_ZONES --region=$REGION"
gcloud container node-pools update $NODE_POOL --cluster=$GKE_CLUSTER_NAME --node-locations=$FAILOVER_ZONES --region=$REGION
...
#resize the cluster with $NUM_NODES per zone
echo "resize the cluster with $NUM_NODES per zone"
